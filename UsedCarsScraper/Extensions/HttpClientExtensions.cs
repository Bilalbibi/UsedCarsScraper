...
public static class HttpClientExtensions
{
    public static T To<T>(this string s)
    {
        ...
        {
            throw new KnownException($"{e.Message} :\n{s}");
        }
    }
    ...
    public static string ToCookiesString(this Dictionary<string, string> cookies)
    {
    ...
    public static async Task<T> To<T>(this Task<Response> task)
    {
        return JsonSerializer.Deserialize<T>((await task).Text);
    }

    public static async Task<HtmlDocument> ToDoc(this Task<Response> r)
    {
        var doc = new HtmlDocument();
        doc.LoadHtml((await r).Text);
        return doc;
    }
    ...
    // }
    public static async Task DownloadFile(this HttpClient client, string url, string path, CancellationToken cancellationToken = default)
    {
        do
        {
            try
    ...
    {
        return await httpClient.DoJson(url, json, HttpMethod.Post, maxAttempts, headers, ct);
    }

    public static async Task<Response> PutJson(this HttpClient httpClient, string url, string json, int maxAttempts = 1, List<KeyValuePair<string, string>> headers = null, CancellationToken ct = new CancellationToken())
    {
        return await httpClient.DoJson(url, json, HttpMethod.Put, maxAttempts, headers, ct);
    }

    private static async Task<Response> DoJson(this HttpClient httpClient, string url, string json, HttpMethod httpMethod, int maxAttempts = 1, List<KeyValuePair<string, string>> headers = null, CancellationToken ct = new CancellationToken())
    {
